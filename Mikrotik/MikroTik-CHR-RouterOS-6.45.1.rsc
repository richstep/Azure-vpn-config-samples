...
# 167.220.152.15 - public IP address associated with my Mikrotik router
# 172.17.1.99/28 - address space of my LAN
# 10.74.0.0/16 - address space of my Azure Vnet

# Add the NAT rule
# YOU MAY NEED TO CHANGE THE PRIORITY OF THE NAT RULE (OR IT DOESN'T WORK).
/ip firewall nat add chain=srcnat action=accept src-address=172.17.1.99/28 \
...
# Add new IPSec Peer
/ip ipsec peer add name=azure-ipsec-peer address=13.211.10.144/32 \
...
/ip ipsec identity add peer=azure-ipsec-peer secret=mykeyblahblahblah35613847

# Add a new IPSec Policy
/ip ipsec policy add src-address=172.17.1.0/28 src-port=any \
dst-address=10.74.0.0/16 dst-port=any protocol=all action=encrypt \
level=require ipsec-protocol=esp tunnel=yes sa-src-address=167.220.152.15 \
